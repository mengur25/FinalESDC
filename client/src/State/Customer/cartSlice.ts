...
>("cart/addItemToCart", async ({ jwt, request }, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      error.response?.data || { message: "Add item to cart failed" }
    );
  }
});
...
>("cart/deleteCartItem", async ({ jwt, cartItemId }, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      error.response?.data || { message: "Delete item to cart failed" }
    );
  }
});
...
>(
  ...
  async ({ jwt, cartItemId, cartItem }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data || { message: "Update item to cart failed" }
      );
    }
  }
);
...
const cartSlice = createSlice({
  ...
  extraReducers:(builder) =>{
    ...
    .addCase(
        fetchUserCart.fulfilled,(state, action: PayloadAction<Cart>) =>{
    ...
    .addCase(
        addItemToCart.fulfilled,(state, action: PayloadAction<CartItem>)=>{
            if(state.cart){
    ...
    .addCase(deleteCartItem.fulfilled, (state, action) =>{
        if(state.cart){
            ...
            state.cart.totlaMrpPrice = mrpPrice;
        }
